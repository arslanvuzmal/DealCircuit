      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-dark-card border border-dark-border p-6 rounded-xl">
        <div>
          ...
          <p className="text-xs text-dark-muted mt-1">
            Track automated executions triggered by Lead Intake, Daily Digest, Failed Event Retry, and Review Completion workflows.
          </p>
        </div>
      </div>
              ...
              {runs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-dark-muted">
                    No workflow runs recorded yet. Workflows log executions automatically upon trigger.
                  </td>
                </tr>
              ) : (
                runs.map((run) => (
                  <tr key={run.id} className="hover:bg-dark-hover/50 transition">
                    <td className="p-3 font-semibold text-dark-bright">{run.workflowName}</td>
                    <td className="p-3 font-mono text-dark-muted">{run.executionId || '-'}</td>
                    <td className="p-3">
                      <span
                    ...
                    <td className="p-3 text-dark-muted">
