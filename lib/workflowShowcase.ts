...
export const workflowShowcases: WorkflowShowcase[] = [
  {
    ...
    businessNarrative: [
      ...
      'Any external form, ad platform, or lead source can POST to this workflow’s public webhook. It normalizes whatever fields arrive, forwards the lead straight into the real DealCircuit qualification API — the same one powering this dashboard — and gets back a genuine category and score. A GPT-5 agent then turns that raw result into a sales-ready brief a rep can act on immediately, not just a category label.',
    ...
    pipeline: [
      ...
      { type: 'step', step: { title: 'Normalize Lead Data', description: 'Maps whatever fields arrive into the exact shape the DealCircuit API expects, with safe fallbacks for missing fields.', color: 'cyan' } },
      {
        type: 'branch',
        title: 'Required Fields Present?',
        paths: [
          { label: 'Valid', color: 'purple', steps: [
            { title: 'Submit to DealCircuit API', description: 'The real production /api/leads endpoint — genuine AI qualification, not a simulation.', color: 'purple' },
          ]},
    ...
    technicalHighlights: [
      ...
      'Genuinely calls the production DealCircuit /api/leads endpoint, so this automation and the dashboard share one real qualification result',
